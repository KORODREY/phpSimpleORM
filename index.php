			...
			<body>
					...
					if ($products !== false && count($products) > 0) {
						foreach ($products as $product) {
							echo "<tr>
								...
								<td>
									<a href='/test/update.php?id=".$product->getId()."'>Редактировать</a>
									<a href='/test/index.php?action=delete&id=".$product->getId()."'>Удалить</a>
								</td>
							</tr>";
						}
					} else {
				...
				if ($products_on_page < $total_products_count) { 
					...
					for ($i = 1; $i <= $pagesCount; $i++) {
						echo "<a href='/test/index.php?p=".$i."'> ".$i." </a>";
					}
				}
